...
float bumpLimit  = 2;
 ...
 #define MIN_WASLESS_SPEED 1.5


float calculatedPosition = 0, aogCompatiblePosition = 6805 ;
float aogCompatiblePositionOld = 6805;
uint32_t steerLoop = 0, oldSteerLoop = 0;
float  kalman = 0.0, oldGyro = 0, gyro = 0, gyroRead = 0, bump = 0; ;
//---------------------------------------------------------------------------
...
/////////////////////////////////////////////
  ...
  // BNO08x definitions
  #define REPORT_INTERVAL 40 //Report interval in ms (same as the delay at the bottom)
  ...
  void loop()
  {
      ...
      else    //ADS1115 Differential Mode ---- WASLESS
      {
          //--------------------------------WAS Less steer-----------------------------
        
       if (bno08x.dataAvailable() == true){

      gyroRead = bno08x.getGyroZ()*-1000;
    bump = bno08x.getAccelY();
     gyro = gyroRead;

    if (gyro>oldGyro + accelRate || gyro < oldGyro - accelRate) 
    {
       if ( gyro > oldGyro && gyro > 0) gyro = oldGyro + accelRate/3;
            }
         if (gyro < oldGyro && gyro < 0)  { gyro = oldGyro - accelRate/3;
            }
     
  if (abs (gyro)<rate && abs(bump) < bumpLimit) {
    kalman = gyro; //simpleKalmanFilter.updateEstimate(gyro);
        oldGyro = gyro;
  }
            if (gpsSpeed>MIN_WASLESS_SPEED) {
            
            calculatedPosition = kalman /gpsSpeed;
            aogCompatiblePosition = (calculatedPosition*100) +6805 ;
            aogCompatiblePositionOld = aogCompatiblePosition;
            }
            else { aogCompatiblePosition= aogCompatiblePositionOld;
            }
  }
              
        steeringPosition = aogCompatiblePosition;
